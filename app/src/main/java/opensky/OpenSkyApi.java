...
public class OpenSkyApi {
	...
	private static class BasicAuthInterceptor implements Interceptor {
		...
		BasicAuthInterceptor(String a, String b) {
			credentials = Credentials.basic(a, b);
		}
	 ...
	 * @param a an OpenSky a
	 * @param b an OpenSky b for the given a
	 */
	public OpenSkyApi(String a, String b) {
		...
		authenticated = a != null && b != null;

        if (authenticated) {
            okHttpClient = new OkHttpClient.Builder()
                    .addInterceptor(new BasicAuthInterceptor(a, b))
                    .build();
        } else {
	...
	private boolean checkRateLimit(REQUEST_TYPE type, long timeDiffAuth, long timeDiffNoAuth) {
        ...
        if (DEBUG && t != null){
            Log.wtf(this.getClass().getSimpleName(), String.format("now - t: %d", now - t));}
	...
	public static class BoundingBox {
		...
		private void checkLongitude(double lon) {
			if (lon < -180 || lon > 180) throw new RuntimeException(String.format("Illegal longitude %f. Must be within [-180, 180]", lon));
		}
	}
